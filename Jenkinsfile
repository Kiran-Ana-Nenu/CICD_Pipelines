                        ...
                        stage('Docker Build (Parallel)') {
                            steps {
                                script {
                                    ...
                                    def dockerPath = "docker"
                                    images.each {
                                        name, image -> buildTasks["Build ${name}"] = {
                                            script {
                                                def dockerFile = ""
                                                switch (name) {
                                                    case "web":
                                                    case "worker-app": dockerFile 
                                                        = "app.Dockerfile"; 
                                                        break case "worker-mail": dockerFile 
                                                        = "mail.Dockerfile"; 
                                                        break case "nginx": dockerFile 
                                                        = "nginx.Dockerfile"; 
                                                        break default : error("❌ Unknown image: ${name}")
                                                }
                                                echo "🔨 Building ${name} -> ${image} using ${dockerFile}"
                                                sh """
              docker build \
                        ...
                        stage('Trivy Scan') {
                            steps {
                                script {
                                    def images = readJSON(text: env.IMAGES)
                                    images.each {
                                        name, image -> echo "🔍 Trivy scanning ${image}"
                                        ...
                                        def total = 0
                                        json.Results?.each {
                                            r -> total += r.Vulnerabilities?.size() ?: 0
                                        }
